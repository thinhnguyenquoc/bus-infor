...
app.controller('HomeCtrl', function ($scope, $timeout, SuggestStation, GetPosition, Search, blockUI, lazyLoadApi) {
            ...
            if (status === google.maps.GeocoderStatus.OK) {
                if (results[1]) {
                    $timeout(function () {
                        ...
                        $scope.rightclick = false;
                        var id = $scope.getSuggestionStation($scope.currentLatLng.lat, $scope.currentLatLng.lng, true);
                    }, 0);
                } else {
            ...
            if (status === google.maps.GeocoderStatus.OK) {
                if (results[1]) {
                    $timeout(function () {
                        ...
                        $scope.rightclick = false;
                        var id = $scope.getSuggestionStation($scope.currentLatLng.lat, $scope.currentLatLng.lng, false);
    ...
    $scope.SuggestionStation = null;

    $scope.getSuggestionStation = function (lat, lng, flag) {
        $scope.SuggestionStation = SuggestStation.getstations(lat, lng);
        $scope.SuggestionStation.then(function (data) {
            if (data != null) {
                alert("Id trạm:" + data.Id + " lat;lng:" + lat + ";" + lng);
                if(flag){
                    $scope.fromPoint.bindPopup("Id trạm:" + data.Id + " lat;lng:" + lat + ";" + lng);
                    $scope.fromLatLng = lat + ";" + lng;
                }
                else{
                    $scope.toPoint.bindPopup("Id trạm:" + data.Id + " lat;lng:" + lat + ";" + lng);
                    $scope.toLatLng = lat + ";" + lng;
                }
            }
            else {
                alert("không tìm thấy trạm");
                if (flag)
                    $scope.fromPoint.bindPopup("Id trạm: " + "-1");
                else
                    $scope.toPoint.bindPopup("Id trạm: " + "-1");
            }
...
app.factory('SuggestStation', function ($http, $q) {
    ...
    SuggestStation.getstations = function (lat, lon) {
        ...
        $http.get(baseUrl + "Assignment2016/SuggestStation?lat=" + lat + "&lng=" + lon)
        .then(function (response) {
