    ...
    $scope.search = function () {
        ...
        $scope.SearchResult.then(function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                if (i < data.length - 1) {
                    if (data[i][0].Type == 1) {
                        detail.push("<i class='fa fa-bus'></i> &nbsp; Đi xe số <b>" + data[i][data[i].length - 1].Route.Code + "</b> từ <b>" + data[i + 1][0].ToStation.Address + "</b> tới <b>" + data[i][0].ToStation.Address + "</b>");
                    }
                    else if (data[i][0].Type == 2) {
                        detail.push("<i class='fa fa-male'></i> &nbsp; <b> Đi bộ </b> từ <b>" + data[i + 1][0].ToStation.Address + "</b> tới <b>" + data[i][0].ToStation.Address + "</b>");
                    }
                }
                else {
                    if (data[i][0].Type == 1) {
                        detail.push("<i class='fa fa-bus'></i> &nbsp; Đi xe số <b>" + data[i][0].Route.Code + "</b> từ <b>" + data[i][data[i].length - 1].FromStation.Address + "</b> tới <b>" + data[i][0].ToStation.Address + "</b>");
                    }
                    else if (data[i][0].Type == 2) {
                        detail.push("<i class='fa fa-male'></i> &nbsp; <b> Đi bộ </> từ <b>" + data[i][data[i].length - 1].FromStation.Address + "</b> tới <b>" + data[i][0].ToStation.Address + "</b>");
                    }
                }

                var dis = 0;
                for (var j = 0; j < data[i].length; j++) {
                    $scope.TotalDistance += data[i][j].Distance;
                    dis += data[i][j].Distance;
                    //sink
                    if (i == 0 && j == 0) {
                        if (data[i][j].RouteId != -1) {                        
                            point = L.marker([data[i][j].ToStation.Lat, data[i][j].ToStation.Lng], { icon: $scope.stationToMarkerResult(data[i][j].Route.Code) }).addTo(map).bindPopup(data[i][j].ToStation.Address);
                        }
                        else {
                            point = L.marker([data[i][j].ToStation.Lat, data[i][j].ToStation.Lng], { icon: $scope.stationToMarkerResult(data[i][j].RouteId) }).addTo(map).bindPopup(data[i][j].ToStation.Address);
                        }
                    }                   
                    else {
                        //sink
                        if (i == data.length - 1 && j == data[data.length - 1].length - 1) {
                            if (data[i][j].RouteId != -1) {
                                point = L.marker([data[i][j].FromStation.Lat, data[i][j].FromStation.Lng], { icon: $scope.stationFromMarkerResult(data[i][j].Route.Code) }).addTo(map).bindPopup(data[i][j].FromStation.Address);
                            }
                            else {
                                point = L.marker([data[i][j].FromStation.Lat, data[i][j].FromStation.Lng], { icon: $scope.stationFromMarkerResult(data[i][j].RouteId) }).addTo(map).bindPopup(data[i][j].FromStation.Address);
                            }
                        }
                        ...
                        if (j == 0) {
                            if (data[i][j].RouteId != -1) {
                                if (data[i - 1][0].RouteId != -1) {
                                    point = L.marker([data[i][j].ToStation.Lat, data[i][j].ToStation.Lng], { icon: $scope.stationChangeMarkerResult(data[i][j].Route.Code) }).addTo(map).bindPopup("Chuyển từ xe số <b>" + data[i][j].Route.Code + "</b> sang <b>" + data[i - 1][0].Route.Code + "</b> <br\> Tại: <b>" + data[i][j].ToStation.Address + "</b>");
                                }
                                else {
                                    point = L.marker([data[i][j].ToStation.Lat, data[i][j].ToStation.Lng], { icon: $scope.stationChangeMarkerResult(data[i][j].Route.Code) }).addTo(map).bindPopup("Chuyển từ xe số <b>" + data[i][j].Route.Code + "</b> sang <b> Đi bộ </b> <br\> Tại: <b>" + data[i][j].ToStation.Address + "</b>");
                                }
                            }
                            else {
                                if (data[i - 1][0].RouteId != -1) {
                                    point = L.marker([data[i][j].ToStation.Lat, data[i][j].ToStation.Lng], { icon: $scope.stationChangeMarkerResult(data[i][j].RouteId) }).addTo(map).bindPopup("Chuyển từ <b> Đi bộ </b> sang <b> xe số " + data[i - 1][0].Route.Code + "</b> <br\> Tại: <b>" + data[i][j].ToStation.Address + "</b>");
                                }
                                else {
                                    point = L.marker([data[i][j].ToStation.Lat, data[i][j].ToStation.Lng], { icon: $scope.stationChangeMarkerResult(data[i][j].RouteId) }).addTo(map).bindPopup("<b> Đi bộ </b> Từ: <b>" + data[i][j].ToStation.Address + "</b>");
                                }
                            }
                        }
                        else {
                            if (data[i][j].RouteId != -1) {
                                point = L.marker([data[i][j].ToStation.Lat, data[i][j].ToStation.Lng], { icon: $scope.stationMarkerResult(data[i][j].Route.Code) }).addTo(map).bindPopup(data[i][j].ToStation.Address);
                            }
                            else {
                                point = L.marker([data[i][j].ToStation.Lat, data[i][j].ToStation.Lng], { icon: $scope.stationMarkerResult(data[i][j].RouteId) }).addTo(map).bindPopup(data[i][j].ToStation.Address);
                            }
                        }
                    }
                ...
                if (data[i][0].Type == 1) {
                    $scope.TotalTime += dis / 1000 / data[i][0].Route.Speed * 60;
                }
                else {
                    $scope.TotalTime += dis / 1000 / 5 * 60;
                }              
            }

            $scope.TotalTime = parseInt($scope.TotalTime, 10);
            ...
            var goal = new google.maps.LatLng(data[data.length - 1][data[data.length - 1].length - 1].FromStation.Lat, data[data.length - 1][data[data.length - 1].length - 1].FromStation.Lng);
            $scope.drawWalk(origin, goal);
            //walk to
            var origin2 = new google.maps.LatLng(data[0][0].ToStation.Lat, data[0][0].ToStation.Lng);
